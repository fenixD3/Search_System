...
void TestFunctionality(const vector<string>& docs, const vector<string>& queries,
...
void Timing()
{
	...
	istringstream doc_input(Join('\n', docs));
	istringstream query_input(Join('\n', queries));
	ostringstream query_out;
	SearchServer srv;
	{
		LOG_DURATION("Updating");
		for (auto i = 0; i < 100000; ++i)
			srv.UpdateDocumentBase(doc_input);
	}
	{
		LOG_DURATION("Query");
		for (auto i = 0; i < 100000; ++i)
			srv.AddQueriesStream(query_input, query_out);
	}
}
